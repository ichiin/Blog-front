...
class App extends Component {
    ...
    addPost = () => {
        ...
        Axios.post(query, parameters, null).then(res => {
    ...
    deletePost = (id) => (event) => {
        ...
        Axios.post(query, {_id: id}, null).then(_ => {
            this.getAllPosts();
        }).catch(err => {
            console.log(err);
        })
        ...
        return (
            <Router>
                ...
                <Switch>
                    ...
                    <Route path="/ArticleList" render={(props) => <ArticleList {...props} db_posts={this.state.db_posts}
                                                                               getAllPosts={this.getAllPosts}
                                                                               deletePost={this.deletePost}/>}/>
                    <Route path="/LogIn" render={(props) => <LogIn {...props} username={this.state.username}
                                                                   password={this.state.password}
                                                                   userLogIn={this.userLogIn}
                                                                   inputHandler={this.inputHandler}/>}/>
                    <Route path="/AddPost"
                           render={(props) => <AddPost {...props} new_post_body={this.state.new_post_body}
                                                       new_post_tags={this.state.new_post_tags}
                                                       new_post_name={this.state.new_post_name}
                                                       inputHandler={this.inputHandler} tagsAdd={this.tagsAdd}
                                                       addPost={this.addPost}
                                                       new_post_tags_array={this.state.new_post_tags_array}
                                                       handleDeleteTag={this.handleDeleteTag}
                                                       new_post_inserted={this.state.new_post_inserted}
                                                       new_post_imgURL={this.state.new_post_imgURL}
                                                       clearNewPost={this.clearNewPost}/>}/>
                    <Route path="/LogOut"
                           render={(props) => <LogOut {...props} logOut={this.logOut} session={this.state.session}/>}/>
                    {this.state.db_posts && this.state.db_posts.length > 0 && this.state.db_posts.map(post => {
                        return (<Route path={`/${post.post_title.replace(/ /g, '-')}`}
                                       render={(props) => <Article {...props} article_title={post.post_title}
                                                                   article_tags={post.post_tags}
                                                                   article_body={post.post_body}
